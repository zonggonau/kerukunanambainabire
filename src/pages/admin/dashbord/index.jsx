...
import { getSumAnggota } from "../../../../lib/API";

export default function Dashbord({ user, data }) {
                ...
                >
                  <li className="flex">
                    <Link href={"/user/profile/" + user.id}>
                      <span className="cursor-pointer inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
                        <svg
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-xs">
                            <span className="px-2 py-1 font-semibold leading-tight text-orange-700 bg-orange-100 rounded-full dark:text-white dark:bg-orange-600">
                              {item.anggota == null
                                ? "required"
                                : item.anggota.pekerjaan_sekarang}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm">
                            {item.anggota == null
...
export async function getServerSideProps(ctx) {
  ...
  if (cookie.admin !== "admin") {
    return {
      redirect: {
        destination: "/user/profile/" + user.id,
      },
    };
  }
  ...
  const res = await fetch(
    process.env.NEXT_PUBLIC_HOST + "/api/users?" + query,
    {
      headers: {
