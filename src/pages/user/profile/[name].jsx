    ...
    <>
      <div className="bg-gray-200">
        <Header
          logout={logout}
          user={
            profile.anggota == null ? profile.username : profile.anggota.nama
          }
                ...
                <ul className="bg-gray-100 text-gray-600 hover:text-gray-700 hover:shadow py-2 px-3 mt-3 divide-y rounded shadow-sm">
                  <li className="flex items-center py-3">
                    {profile.anggota == null ? (
                      ...
                      <Link
                        href={`/user/profile/document/${user.id}`}
                        target="_blank"
                      >
                        Download
                      </Link>
...
export async function getServerSideProps(ctx) {
  ...
  if (user.admin !== "admin") {
    if (ctx.params.name !== cookie.id) {
      return {
        redirect: {
          destination: "/user/profile/" + cookie.id,
        },
      };
    }
    // else {
    //   return {
    //     redirect: {
    //       destination: "/user/profile/" + ctx.params.name,
    //     },
    //   };
    // }
  }
